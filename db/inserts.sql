...
BEGIN
    ...
    VALUES	('Litro',               'l',    3, (SELECT id FROM Sensors WHERE name = Volume), 'TRUE', 'pv'),
	        ...
	        ('Celsio',              'ºC',   2, (SELECT id FROM Sensors WHERE name = Temperature), FALSE', 'pv * -273.15'),
	        ('Farenhit',            'ºF',   2, SELECT id FROM Sensors WHERE name = Temperature), 'FALSE', '(pv * -273.15) * 9 / 5 + 32'),
